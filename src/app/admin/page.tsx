...
import { UploadCloud, Loader2, Trash2, LogOut, CheckCircle2, Pencil, X } from "lucide-react";
...
export default function AdminPage() {
  ...
  const [editingProperty, setEditingProperty] = useState<any>(null); // Estado para edição
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex justify-between items-center bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
          <h1 className="text-xl font-black text-[#2D3436] uppercase tracking-tighter">
            {editingProperty ? "Editar" : "Novo"} <span className="text-[#BF953F]">Imóvel</span>
          </h1>
          <div className="flex items-center gap-4">
            {editingProperty && (
              <button 
                onClick={() => { setEditingProperty(null); setImages([]); }}
                className="text-gray-400 text-[10px] font-black uppercase flex items-center gap-1 hover:text-black transition-all"
              >
                <X size={14} /> Cancelar
              </button>
            )}
        ...
        <form
          ...
          action={async (fd) => {
            try {
              ...
              if (editingProperty) {
                await updateProperty(editingProperty.id, fd);
              } else {
        ...
        >
          ...
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex flex-col gap-2">
                ...
                <select 
                  name="type" 
                  key={editingProperty?.id} // Força o reset do select ao trocar de imóvel
                  defaultValue={editingProperty?.type || ""}
                  className="p-4 bg-gray-50 rounded-2xl outline-none font-bold text-gray-700 focus:ring-2 focus:ring-[#BF953F] appearance-none" 
                  required
                >
              ...
              <div className="flex flex-col gap-2">
                ...
                <input name="title" defaultValue={editingProperty?.title || ""} className="p-4 bg-gray-50 rounded-2xl outline-none font-bold text-gray-700 focus:ring-2 focus:ring-[#BF953F]" placeholder="Ex: Sobrado Moderno" required />
              </div>

              <div className="flex flex-col gap-2">
                <label className="text-[10px] font-black uppercase text-gray-500 ml-1">Preço Venda (R$)</label>
                <input name="price" type="number" defaultValue={editingProperty?.price || ""} className="p-4 bg-gray-50 rounded-2xl outline-none font-bold text-gray-700 focus:ring-2 focus:ring-[#BF953F]" placeholder="0,00" required />
              </div>

              <div className="flex flex-col gap-2">
                <label className="text-[10px] font-black uppercase text-gray-500 ml-1">Bairro</label>
                <input name="neighborhood" defaultValue={editingProperty?.neighborhood || ""} className="p-4 bg-gray-50 rounded-2xl outline-none font-bold text-gray-700 focus:ring-2 focus:ring-[#BF953F]" placeholder="Ex: Jardim América" required />
              </div>
              ...
              <div className="md:col-span-2 grid grid-cols-2 md:grid-cols-5 gap-4 mt-2">
                <div className="flex flex-col gap-2">
                  ...
                  <input name="area" type="number" defaultValue={editingProperty?.area || ""} className="p-4 bg-gray-50 rounded-2xl outline-none font-bold text-gray-700 focus:ring-2 focus:ring-[#BF953F]" required />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black uppercase text-gray-400 ml-1 tracking-widest">Dorms</label>
                  <input name="bedrooms" type="number" defaultValue={editingProperty?.bedrooms || ""} className="p-4 bg-gray-50 rounded-2xl outline-none font-bold text-gray-700 focus:ring-2 focus:ring-[#BF953F]" required />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black uppercase text-gray-400 ml-1 tracking-widest">Suítes</label>
                  <input name="suites" type="number" defaultValue={editingProperty?.suites || ""} className="p-4 bg-gray-50 rounded-2xl outline-none font-bold text-gray-700 focus:ring-2 focus:ring-[#BF953F]" required />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black uppercase text-gray-400 ml-1 tracking-widest">Banheiros</label>
                  <input name="bathrooms" type="number" defaultValue={editingProperty?.bathrooms || ""} className="p-4 bg-gray-50 rounded-2xl outline-none font-bold text-gray-700 focus:ring-2 focus:ring-[#BF953F]" required />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black uppercase text-gray-400 ml-1 tracking-widest">Vagas</label>
                  <input name="garage" type="number" defaultValue={editingProperty?.garage || ""} className="p-4 bg-gray-50 rounded-2xl outline-none font-bold text-gray-700 focus:ring-2 focus:ring-[#BF953F]" required />
                </div>
              </div>

              <div className="md:col-span-2 flex flex-col gap-2">
                <label className="text-[10px] font-black uppercase text-gray-500 ml-1">Descrição</label>
                <textarea name="description" defaultValue={editingProperty?.description || ""} className="p-4 bg-gray-50 rounded-2xl outline-none font-medium text-gray-700 focus:ring-2 focus:ring-[#BF953F] h-32 resize-none" />
              </div>
            </div>
          </div>

          {/* SEÇÃO 3: BOTÃO DINÂMICO */}
          <button
            type="submit"
            disabled={uploading}
          ...
          >
            {uploading ? "Aguarde o Upload..." : editingProperty ? "Salvar Alterações" : "Publicar Imóvel no Site"}
          </button>
        </form>
